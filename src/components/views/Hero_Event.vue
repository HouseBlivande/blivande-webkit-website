<template>
  ...
  >
    <div
      ...
      v-if="data.image"
      :style="{ flexBasis: data.image.width + 'px' }"
    >
    ...
    <div class="hero_event md:hero_event-md">
      <div class="hero_title flex flex-col md:hero_title-md">
        ...
        <a
          v-if="data.tag"
          class="hero_tag"
          :style="uiStyle('tag', data.style)"
          :href="'http://edgeryders.eu/tags/' + data.tag"
          target="_blank"
          >#{{ data.tag }}</a
        >
        <h3
      ...
      <div
        ...
        v-html="data.text.content"
        :style="textStyle('paragraph', data.style)"
      ></div>
      <div class="hero_event_info md:hero_event_info-md">
        <div class="flex flex-col md:flex-row md:border-r">
          <a
            class="action md:action-md"
            v-for="item in data.text.buttons"
            :style="uiStyle('action', data.style)"
            :href="item.url"
            :key="item.url"
          >
            {{ item.text }}
        ...
        <div class="event_info_container">
          <div
            class="event_info md:event_info-md"
            v-if="$globals.event"
            :style="uiStyle('highlight', data.style)"
          >
            <span
              class="glyph location"
              :style="uiStyle('highlight', data.style)"
            ></span>
            <div v-if="$globals.event.timezone">
              {{ $globals.event.timezone }}
            </div>
          ...
          >
            ...
            <div v-if="$globals.event.start">
              ...
              <span v-if="$globals.event.end"
                >to {{ $globals.event.end | formatDate }}</span
              >
            </div>
