<template>
  <div class="block_text md:block_text-md">
    <div
      ...
      class="text_container md:text_container-md"
    >
      <div
        v-for="(text, index) in content"
        v-html="text"
        class="text"
        :style="elementStyle(stylesheet, 'text')"
        :key="index"
      ></div>
    </div>
    <div
...
export default {
  ...
  created() {
    if (
      ...
      (this.data.includes("/") && this.isNumeric(this.data.split("/")[0]))
    ) {
      var postIndex = 0;
      ...
      axios
        .get(`${this.$globals.config.baseUrl}/t/${this.data}.json`)
        .then(({ data }) => {
