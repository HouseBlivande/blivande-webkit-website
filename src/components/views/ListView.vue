<template>
  <div
    class="list_view md:list_view-md"
    :class="{ 'list_view-md': $mq == 'md' }"
    :style="elementStyle(config.style, 'wrapper')"
  >
    ...
    <div class="list_selected" :class="{ active: slideIn }">
      <div
        class="go_back"
        :class="{ overlay: selected && selected.image_url }"
        @click="showItem()"
      >
        Go back
      </div>
    ...
    showDate(item, index) {
      if (item.event) {
        if (
          ...
          (this.filteredItems[index - 1].event &&
            this.formatDate(this.filteredItems[index - 1].event.start) !==
    ...
    filteredItems() {
      ...
      if (this.config && this.config.sort_by) {
        if (this.config.sort_by.property == "event_date") {
          ...
          filtered = this.sortBy(
            this.data,
            this.config.sort_by.property,
            this.config.sort_by.order
          );
        }
      }
