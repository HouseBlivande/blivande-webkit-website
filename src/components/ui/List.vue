  ...
  <div class="mobile_list" :class="type">
    <div class="list_container" :class="{ active: active }">
      <div class="list_header" v-if="header">
        <h3>{{ header }}</h3>
        <div
          class="toggle search_list_mobile mr-3"
          @click="toggleSearch()"
          :class="{ active: active_search }"
        ></div>
      </div>
      ...
      <ul class="list_scroll">
        ...
        >
          <div
            class="item_time_marker"
            v-if="item.event && showDate(item, index)"
          >
          ...
          <div class="item_text">
            ...
            <p v-if="description" class="item_description">
              {{ item[description] }}..
            </p>
            <p v-if="item.event" class="event_time">
              Starts {{ formatTime(item.event.start) }}
            </p>
          </div>
        </li>
      </ul>
    ...
    <div class="list_slide" :class="{ active: active }">
      <div
        v-if="selected"
        class="list_back"
        :class="{ float: selected[image] && type == 'event' }"
        @click="removeActive()"
      >
        ...
        <div class="share_links mr-4" v-if="share">
          ...
          <a
            ...
            :href="
              'https://www.facebook.com/sharer/sharer.php?u=' + selected[url]
            "
          ></a>
          <a
            class="social email"
            :href="
              'mailto:?subject=' + selected[title] + '&body=' + selected[url]
            "
          ></a>
        </div>
      </div>
      <div v-if="selected" class="list_item_info">
        <div
          v-if="selected[image] && type == 'event'"
          class="event_image"
          :style="{ backgroundImage: 'url(' + selected[image] + ')' }"
        ></div>
        <div class="list_event_title" v-if="type == 'event'">
          <h3>{{ selected[title] }}</h3>
          <p v-if="selected.event">
            {{ formatDate(selected.event.start) }} at
            {{ formatTime(selected.event.start) }}
          </p>
        </div>
        <p class="list_item_description">{{ selected[info] }}</p>
        <a
          v-if="url && type == 'event'"
          class="list_event_url"
          :href="selected[url]"
          target="_blank"
          >Discuss this event</a
        >
        <a
          v-if="url && selected[url] && type != 'event'"
          class="list_item_url mt-4"
          :href="selected[url]"
          target="_blank"
          >Read More</a
        >
...
export default {
  ...
  methods: {
    ...
    showDate(item, index) {
      if (item.event) {
        if (
          !this.filteredList[index - 1] ||
          (this.filteredList[index - 1].event &&
            this.formatDate(this.filteredList[index - 1].event.start) !==
              this.formatDate(item.event.start))
        ) {
          return true;
        } else {
          return false;
