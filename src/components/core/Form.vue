<template>
  <div
    class="section w-full mt-6 mb-6 md:section-md"
    :style="[textStyle('paragraph', data.style), containerStyle(data.style)]"
    id="form"
  >
    <div
      class="md:flex flex-col mx-auto items-start"
    ...
    >
      <div class="w-full">
        ...
        <div
          :class="getClass('paragraph', data.style)"
      ...
      <div class="form mt-4" v-if="!submitted">
        <div class="field" v-for="(field, index) in data.fields" :key="index">
          <h4 :class="{ title_underline: field.type == 'create_account' }">
            {{ field.title }}
            <span
          ...
          />
          <div
            class="flex items-center mt-4 px-1"
            v-if="field.type == 'create_account'"
          >
        ...
        <div class="border p-3 bg-gray-100 rounded-lg" v-if="errors.length > 0">
          Please
          <span v-if="errors[0] !== 'create_account'">enter a valid</span>
