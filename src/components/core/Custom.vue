  ...
  <div class="section md:section-md" :style="containerStyle(data.style)">
    ...
    >
      <div
        ...
        :class="data.style && data.style.title && data.style.title.class"
        v-if="data.title"
      >
        <h3
          :class="titleClassSize(data.style)"
          class="leading-tight w-full md:leading-normal"
        >
          {{ data.title }}
        </h3>
      </div>
      <div class="content_wrapper md:content_wrapper-md">
        <div
          ...
          :style="{ 'flex-basis': data.image.width + 'px' }"
        >
        ...
        <div>
          <div
            v-if="Array.isArray(data.text.content)"
            class="flex flex-col md:flex-row justify-between"
            :class="{ 'md:flex': data.text.layout == 'horizontal' }"
          >
            <div
              v-for="(text, index) in data.text.content"
              v-html="text"
              :style="textStyle('paragraph', data.style)"
              class="section_text md:section_text-md flex-grow"
              :key="index"
            ></div>
          </div>
          <div
